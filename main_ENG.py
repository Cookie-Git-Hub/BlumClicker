...
def choose_window():
    ...
    choice = simpledialog.askstring("Select Telegram window:", "Enter the window number\n" + "\n".join(f"{i}: {window}" for i, window in enumerate(windows)))
    ...
    while True:
        if keyboard.is_pressed('S'):
            ...
            if paused:
                ...
                print("Press 'S' to stop the clicker.")
            time.sleep(0.3)
...
def main():            
    ...
    if not check:
        print(f"\nWindow {WINDOW_NAME} not found! Try to select another window.")
        window_name = choose_window()
    else:
        print(f"\nWindow {WINDOW_NAME} found!\nPress 'S' to start.")
    ...
    if window_name:
        ...
        print("No valid window selected. Exiting.")
